 const sutras_ = [sutra_1,sutra_2,sutra_3,sutra_4,sutra_5,sutra_6,sutra_7,sutra_8,sutra_9,sutra_10,
  ...
  function watermark(settings) {
    ...
    function displayChpterItem( targetChapter , targetParagraph){
        ...
        document.getElementById("sutra_title").innerHTML = `${targetChapter} ${sutra_titles[targetChapter-1][1]} <a href='#' onclick='audioPlay();return false;'>/</a> Chapter <a href='#' onclick='audioPlay();return false;'>${targetParagraph}</a>`;
        var body_data = "";
        for(var k = 0;k<size_sutra;k++){
            // console.log(`${sutraENs[k]}`)
            // console.log(`${sutraCNs[k]}`)
			const dic_text = BuildDictionaryItem(sutraENs[k]);
            const dictionary_item= `<p class='dictionary_text' id="dic_item_${k}" style='display:none;'> ${dic_text} </p>`;
